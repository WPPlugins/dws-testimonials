...
Copyright 2012 Saad Siddique (Dynamic Web Solutions)  (email : jigzyy@gmail.com)
...
$pluginVersion = '1.0.0';
$pluginName = 'DWS Testimonial';
$pluginShortName = str_replace(' ','',strtolower($pluginName));
$pluginSlugName = str_replace(' ','-',strtolower($pluginName));

//Creating New Post Type.
$dwsTestimonials = new DWS_Post_Type($pluginName, 'Testimonial', 'Testimonials', plugins_url( 'images/testimonial.png' , __FILE__ ), 'testimonials', 'page', 31, array('title','editor','thumbnail'));
...
function create_testimonial_page(){
    ...
    $page = get_page_by_name('Testimonials');
    if (!empty($page)) {
...
function dwstestimonial_func($atts) {
   ...
   if(have_posts()):
    if($excerpt == "yes"):
        ...
        return 'We are still adding testimonials. Please check back later.';
   endif;
   wp_reset_query();
   create_testimonial_page();
}
